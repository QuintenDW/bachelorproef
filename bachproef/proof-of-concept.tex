...
\begin{itemize}
    \item De gemiddelde verzoeken per seconde die het kan verwerken.
    \item Het gemiddelde CPU-gebruik.
    \item Het maximale geheugengebruik.
...
Hierbij zit elke aplicatie in zijn eigen containervirtualisatie waarbij aan de hand van een dockerfile deze telkens wordt opgezet.
...
Voor de HTTP server is er nog een bijkomende
...
Hiernaast wordt dan ook een dockerfile gebruikt om de server te starten. De dockerfile voor de Node.js server is te vinden in \ref{code:dockernode}.
\begin{listing}[H]
  \centering
  \begin{minted}[bgcolor=bg,
      fontfamily=tt,
